 ...
 */
public final class TagValueConverterOld
{
    private static final LogFactory LOGGER = LogFactory.getLogger(TagValueConverterOld.class);
    ...
    private static String decodeUserComment(byte[] data)
    {
        ...
        {
            // 1. Extract exactly the first 8 bytes as a raw string, preserving all nulls.
            // DO NOT call .trim() here, or it will alter the fixed 8-byte alignment.
            String headerKey = new String(data, 0, ENCODING_HEADER_LENGTH, StandardCharsets.US_ASCII);

            // 2. Slice out the payload bytes that come after the 8-byte header
            byte[] payloadBytes = Arrays.copyOfRange(data, ENCODING_HEADER_LENGTH, data.length);

            // 3. Match against your universal padded map
            Charset charset = ENCODING_MAP.get(headerKey);

            if (charset == null)
            {
                // Fallback if a non-standard camera doesn't pad with nulls
            ...
            // 5. Terminate the string at the first null byte found in the payload
