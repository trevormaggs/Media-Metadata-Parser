...
final class SummaryDialogFactory2
{
    ...
    static void show(Window owner, TextField targetText, ObservableList<ProcessedFileRecord> completedFileRecords)
    {
                    ...
                    public void handle(MouseEvent event)
                    {
                        if (!row.isEmpty())
                        {
                            ...
                            debouncer.request(new Runnable()
                            {
                                ...
                                public void run()
                                {
                                    // Position popup to the right of the dialog window
        ...
        // Cancel pending preview requests and release thumbnail resources
    ...
    static void showThumbnail(Window owner, TextField targetText, ObservableList<ProcessedFileRecord> completedFileRecords)
    {
        Path targetDir = null;
        final HoverDebouncer debouncer = new HoverDebouncer(120);
                ...
                openFolderItem.setOnAction(new EventHandler<ActionEvent>()
                {
                    ...
                    public void handle(ActionEvent event)
                    {
                        ...
                        if (record != null && targetDir != null)
                        {
                            ...
                            {
                                UtilsJavaFX.launchPopup(table.getScene().getWindow(), "File Error", "Unable to open directory location:\n" + exc.getMessage(), AlertType.ERROR);
                            }
                        }
                    }
                });
                    ...
                    public void handle(MouseEvent event)
                    {
                        if (event.getClickCount() == 2 && !row.isEmpty())
                        {
                            ...
                            if (record != null && targetDir != null)
                            {
                                ...
                                {
                                    UtilsJavaFX.launchPopup(table.getScene().getWindow(), "File Error", "Unable to open target file:\n" + exc.getMessage(), AlertType.ERROR);
                                }
                            }
                        }
                    }
