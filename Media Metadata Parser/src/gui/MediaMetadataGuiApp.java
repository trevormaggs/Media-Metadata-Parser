 ...
 */
public class MediaMetadataGuiApp extends Application implements EventHandler<ActionEvent>
{
    private Stage stage;
    ...
    {
        final TextField sourceText = MainViewPane.getById(pane, MainViewPane.SRCID);
        ...
        final CheckBox showMetadataCheck = MainViewPane.getById(pane, MainViewPane.SHWID);
    ...
    private void attachActionHandlers()
    {
        ...
        viewPane.getSelectFiles().setOnAction(this);
    ...
    private void executeBatchProcess()
    {
        ...
        if (logArea != null)
        {
            ...
            workerTask.setFileSummaryListener(new BiConsumer<String, Object>()
            {
                @Override
    ...
    {
        final TextArea logArea = (TextArea) viewPane.getClearLogBtn().getUserData();
    ...
    private void resetControlStates(final Label progressLabel)
    {
        ...
        Button cancelBtn = viewPane.getCancelBtn();
        Button copyLogBtn = viewPane.getCopyLogBtn();
        final ProgressBar progressBar = viewPane.getProgressBar();
    ...
    private void showSummaryDialog(Window ownerWindow)
    {
        ...
        TableView<FileSummaryRecord> table = new TableView<FileSummaryRecord>();
        table.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY);

        TableColumn<FileSummaryRecord, String> sourceCol = new TableColumn<FileSummaryRecord, String>("Source File");
        sourceCol.setCellValueFactory(new Callback<CellDataFeatures<FileSummaryRecord, String>, ObservableValue<String>>()
        {
            @Override
            public ObservableValue<String> call(CellDataFeatures<FileSummaryRecord, String> cell)
            {
                return cell.getValue().sourceNameProperty();
            }
        });
        sourceCol.setPrefWidth(200);

        TableColumn<FileSummaryRecord, String> targetCol = new TableColumn<FileSummaryRecord, String>("Target File");
        targetCol.setCellValueFactory(new Callback<CellDataFeatures<FileSummaryRecord, String>, ObservableValue<String>>()
        {
            @Override
            public ObservableValue<String> call(CellDataFeatures<FileSummaryRecord, String> cell)
            {
                return cell.getValue().targetNameProperty();
            }
        });
        targetCol.setPrefWidth(200);

        TableColumn<FileSummaryRecord, String> statusCol = new TableColumn<FileSummaryRecord, String>("Status");
        statusCol.setCellValueFactory(new Callback<CellDataFeatures<FileSummaryRecord, String>, ObservableValue<String>>()
        {
            @Override
            public ObservableValue<String> call(CellDataFeatures<FileSummaryRecord, String> cell)
            {
                return cell.getValue().statusProperty();
            }
        });
