...
public class RiffHandler implements ImageHandler
{
    ...
    /**
     * Iterates sequentially through the RIFF container and parses WebP chunks.
     *
     * <p>
     * Image bitstream headers are decoded for {@code VP8}, {@code VP8L}, and {@code VP8X} chunks,
     * while all other chunk types are stored or skipped according to the configured chunk filter.
     * </p>
     *
     * @param reader
     *        the binary reader positioned immediately after the WebP signature
     * @param totalChunkSize
     *        the total size of the RIFF container, in bytes
     *
     * @throws IOException
     *         if an I/O error occurs while reading the stream
     * @throws IllegalStateException
     *         if a chunk exceeds the container bounds or a critical bitstream header is malformed
     */
    private void parseChunks(BinaryInput reader, long totalChunkSize) throws IOException
    {
        ...
        while (reader.getCurrentPosition() + CHUNK_HEADER_SIZE <= totalChunkSize)
        {
            ...
            if (requiredChunks == null || requiredChunks.contains(chunkType))
            {
                ...
                {
                    /*
                     * To conserve memory, only the first 10 bytes are read. This budget is
                     * sufficient to decode the minimum header information required by all three
                     * WebP bitstream formats (VP8, VP8L, and VP8X) without allocating the whole
                     * payload.
                     */
