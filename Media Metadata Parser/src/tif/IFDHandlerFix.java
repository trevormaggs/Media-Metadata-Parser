...
/**
 * This handler performs the heavy lifting of parsing TIFF-based metadata structures (Standard TIFF,
 * EXIF in JPEGs, and DNG) by interpreting Image File Directories (IFDs).
 *
 * <p>
 * It performs a deep recursive traversal of the IFD chain, following pointers to sub-directories
 * such as {@code EXIF}, {@code GPS}, and {@code INTEROP}. It adheres to the TIFF 6.0 specification
 * for directory structures and tag entry parsing.
 * </p>
 *
 * <p>
 ...
 * @version 1.4
 */
public class IFDHandlerFix implements ImageHandler, AutoCloseable
{
    private static final LogFactory LOGGER = LogFactory.getLogger(IFDHandlerFix.class);
     ...
     */
    public IFDHandlerFix(ByteStreamReader reader)
    {
        this.reader = reader;
    }
     ...
     */
    public IFDHandlerFix(Path fpath) throws IOException
    {
     ...
     */
    public IFDHandlerFix(byte[] payload)
    {
    ...
    public boolean parseMetadata() throws IOException
    {
        ...
        if (directoryList.size() > 1)
        {
            ...
            {
                // Updated comparison target from IFD_ROOT_DIRECTORY to matching context context
    ...
    private boolean navigateImageFileDirectory(DirectoryIdentifier dirType, long startOffset) throws IOException
    {
        ...
        long currentOffset = reader.getCurrentPosition(); // Verified matching contract hook
