...
/**
 * Defines Adobe Digital Negative (DNG) tags used in TIFF-based image files.
 *
 * <p>
 * This enumeration covers Adobe DNG versions 1.1 through 1.7, including raw sensor calibration,
 * colour transforms, lens correction, depth information, semantic masks, and JPEG XL (JXL)
 * extensions.
 * </p>
 *
 * <p>
 * Although these tags are typically stored in the root IFD, many are also valid in SubIFDs
 * containing raw image data.
 * </p>
 *
 * @author Trevor Maggs
 * @version 1.2
...
public enum TagIFD_DNG implements Taggable
{
    ...
    IFA_MASK_SUB_LEVELS(0xC7EF, "Mask Sub Levels", TagHint.HINT_INTEGER),
    ...
    /** Static cache allowing O(1) identification lookups by Tag ID number. */
    private static final Map<Integer, TagIFD_DNG> ID_MAP = new HashMap<>();

    static
    {
        for (TagIFD_DNG tag : values())
        {
            ID_MAP.put(tag.numID, tag);
        }
    }
    ...
    /**
     * Resolves an internal numerical ID directly to its designated enum value representation.
     *
     * @param id
     *        the numerical Tag ID value
     * @return the associated {@link TagIFD_DNG} instance, or {@code null} if unknown
     */
    public static TagIFD_DNG findById(int id)
    {
    ...
    /**
     * Converts known DNG values into human-readable text.
     *
     * @param val
     *        the tag value
    ...
    public String translate(Object val)
    {
        switch (this)
        {
            ...
            case IFD_PROFILE_TONE_CURVE:
                if (val != null && val.getClass().isArray()) return "[" + Array.getLength(val) + " elements]";
            break;
            ...
            case IFD_PROFILE_LOOK_TABLE_ENCODING:
                return translateProfileHueEncoding(val);

            case IFD_COLORIMETRIC_REFERENCE:
                return translateColorReference(val);

            case IFD_DEPTH_FORMAT:
                return translateDepthFormat(val);
    ...
    /**
     * Translates a preview color space value.
     *
     * @param val
     *        the raw tag value
     * @return a text interpretation of the colour space, or an unknown/fallback representation
     */
    private String translatePreviewColorSpace(Object val)
    {
    ...
    /**
     * Translates a profile embed policy value.
     *
     * @param val
     *        the raw tag value
     * @return a text interpretation of the embed policy, or an unknown/fallback representation
     */
    private String translateProfileEmbedPolicy(Object val)
    {
        if (val instanceof Number)
        {
    ...
    /**
     * Translates profile encoding values.
     *
     * <p>
     * Covers both {@code ProfileHueSatMapEncoding} and {@code ProfileLookTableEncoding}.
     * </p>
     *
     * @param val
     *        the raw tag value
     * @return a text interpretation of the reference space encoding, or an unknown/fallback
     *         representation
     */
    private String translateProfileHueEncoding(Object val)
    {
        if (val instanceof Number)
        {
    ...
    /**
     * Translates a colourimetric reference value.
     *
     * @param val
     *        the raw tag value
     * @return a text interpretation of the reference coordinate space, or an unknown/fallback
     *         representation
     */
    private String translateColorReference(Object val)
    {
        if (val instanceof Number)
        {
    ...
    /**
     * Translates a depth format value.
     *
     * @param val
     *        the raw tag value
     * @return a text interpretation of the depth allocation properties, or an unknown/fallback
     *         representation
     */
    private String translateDepthFormat(Object val)
    {
        if (val instanceof Number)
        {
    ...
    /**
     * Translates a depth measurement type.
     * 
     * @param val
     *        the raw tag value
     * @return a text interpretation of the structural depth perspective metric, or an
     *         unknown/fallback representation
     */
    private String translateDepthMeasureType(Object val)
    {
        if (val instanceof Number)
        {
    ...
    /**
     * Translates a {@code DepthUnits} value into a descriptive string.
     *
     * @param val
     *        the raw tag value
     * @return a text interpretation of the unit definition scalar, or an unknown/fallback
     *         representation
     */
    private String translateDepthUnits(Object val)
    {
        if (val instanceof Number)
        {
