...
public class HeifPropertyInjector
{
    ...
    private void incrementIpmaCount(byte[] newData, ItemPropertyAssociationBox ipma, int targetID, int added, int ipcoAt, int ipmaAt, int pLen, int aLen)
    {
        ...
        /* Skip Box Header(8) + FullBox(4) + EntryCount(4) within IPMA */
     ...
     * </p>
     * <ul>
     *   <li><b>Field Position Shift:</b> If the iloc box itself was located after an injection point, 
     *   the physical address where we write the offset must be shifted.</li>
     *   <li><b>Offset Value Shift:</b> If the data (EXIF, mdat, etc.) that the offset points to has 
     *   moved down the file, the value stored in that field must be incremented.</li>
     * </ul>
     *
     * @param handler
     ...
     * 
     * @param ipma
     *         the parsed ItemPropertyAssociationBox structure
     * @param targetID
     *         the item ID whose layout boundary is being found
     * @return the physical file offset suitable for injecting new associations
     * @throws RuntimeException if the target ID cannot be found in the IPMA structural registry
     */
     ...
     * @return byte array containing the raw 'imir' box structure
     */
     ...
     * @return byte array containing the raw 'clap' box structure
     */
     ...
     * @return byte array containing the raw 'pasp' box structure
     */
    ...
    /**
     * Execution entry point for parsing and running the property injection transformation pipeline.
     * 
     * @param args 
