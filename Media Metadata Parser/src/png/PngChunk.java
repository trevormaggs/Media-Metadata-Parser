 ...
 *
 * <p>
 * PNG chunks consist of a length (4 bytes), type (4 bytes), data (0-n bytes), and a CRC (4 byte s).
...
{
    private static final int CHUNK_HEADER_LENGTH = 8; // 4 bytes Length + 4 bytes Type
    ...
    /**
     * Returns a four-byte CRC computed on the preceding bytes in the chunk, excluding the length
     * field.
     *
     ...
     * file offset position (4 bytes for Length and 4 bytes for Type).
     *
     * @return the offset pointing to the start of the data segment
     */
    public long getDataOffset()
    {
        return chunkOffset + CHUNK_HEADER_LENGTH;
    }
     ...
     */
    public void printProperties(PropertyListener consumer)
    {
        ChunkType type = getType();

        if (type == ChunkType.UNKNOWN)
        {
            String nameStr = new String(typeBytes, java.nio.charset.StandardCharsets.US_ASCII);
            consumer.accept("Unknown Chunk (" + nameStr + ")", "Binary Payload [" + payload.length + " bytes]");
        }

        else if (!type.isTextual())
        {
            consumer.accept(type.getDescription(), "Binary Payload [" + payload.length + " bytes]");
        }
    }
     ...
     * @return true if equal in all fields and file position
     */
    @Override
    ...
    public int hashCode()
    {
        int result = Objects.hash(length, crc, chunkOffset);
